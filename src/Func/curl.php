 ...
 * @throws \Exception
 */
function curl_get($url, $data = null, array $curl_option = []): array
{
    if ($data) {
 ...
 * @throws \Exception
 */
function curl_post($url, $data = null, array $curl_option = []): array
{
 ...
 * @throws \Exception
 */
function curl_put($url, $data, array $curl_option = []): array
{
    $ch = curl_instance($url, curl_merge_options([
        CURLOPT_POSTFIELDS    => curl_data2str($data),
 ...
 * @throws \Exception
 */
function curl_delete($url, $data, array $curl_option = []): array
{
    $ch = curl_instance($url, curl_merge_options([
        CURLOPT_POSTFIELDS    => curl_data2str($data),
 ...
 * @return array [head, body, ...] curl_getinfo信息
 */
function curl_execute($ch): array
{
 ...
 * @throws \Exception
 */
function curl_instance(string $url, array $curl_option)
{
    //use ssl
 ...
 * @return string
 */
function curl_data2str($data): string
{
    if (is_scalar($data)) {
        return (string)$data;
    }
        ...
        if (is_assoc_array($data)) {
            foreach ($data as $k => $v) {
                ...
                if (is_scalar($v)) {
                    ...
                    throw new \LogicException('Data type no support(more than 3 dimension array no supported)');
                }
            }
        } else {
            $d += $data;
    ...
    throw new \LogicException('Data type no supported');
}
 ...
 * @return array|null
 */
function curl_print_option($options, bool $as_return = false): ?array
{
 ...
 * @return array
 */
function curl_merge_options(...$options): array
{
    $ret     = [];
